defmodule RumblReact.AuthTest do
	use RumblReact.ConnCase
	alias RumblReact.Auth

	setup %{conn: conn} do
		conn =
			conn
			|> bypass_through(RumblReact.Router, :browser)
			|> get("/")
		{:ok, %{conn: conn}}
	end
	...
	test "authenticate_user continues when the current_user exists", %{conn: conn} do
		conn =
			conn
			|> assign(:current_user, %RumblReact.User{})
	...
	test "login puts the user in the session", %{conn: conn} do
		login_conn =
			conn
			|> Auth.login(%RumblReact.User{id: 123})
			|> send_resp(:ok, "")
	...
	test "logging drops the session", %{conn: conn} do
		logout_conn = 
			conn
